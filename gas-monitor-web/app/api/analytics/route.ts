...
export async function GET(req: Request) {
  ...
  const [allOrders, recentOrders] = await Promise.all([
    prisma.order.findMany({ where: { consumerId: auth.sub }, select: { totalAmount: true, status: true } }),
    prisma.order.findMany({
      where: { consumerId: auth.sub, createdAt: { gte: since }, status: { in: REVENUE_STATUSES } },
      select: { totalAmount: true, createdAt: true }
    })
  ]);
